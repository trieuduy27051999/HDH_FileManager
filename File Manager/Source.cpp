...
string DateToString(WORD Date) {
    DWORD nYear = (Date >> 9) + 1980;
    DWORD nMonth = (Date << 7);
    nMonth = nMonth >> 12;
    WORD nDay = (Date << 11);
    nDay = nDay >> 11;
    return to_string(nDay) + '/' + to_string(nMonth) + '/' + to_string(nYear);
}
...
int ReadSector2(LPCWSTR  drive, int readPoint, int byte, BYTE sector[512])
{
    int retCode = 0;
    DWORD bytesRead;
    HANDLE device = NULL;
    ...
    if (!ReadFile(device, sector, byte, &bytesRead, NULL))
    {
        printf("ReadFile: %u\n", GetLastError());
    }
    else
...
void showBootSectorInformation(BPB _bpb) {
    ...
    printf("Number of entry for RDET: %d \n", _bpb.BPB_RootEntCnt);
            ...
            for (int index = 0; index < (512 / 32); index++) {
                ...
                if (RDETEntry[0] == 0x00) {
                    ...
                    if (RDETEntry[11] == 0x0f) {
                        ...
                        /*char szLongFileName[MAX_LENGTH];
                        char szLongFileNameTemp[MAX_LENGTH];
                        memset(szLongFileName, 0, MAX_LENGTH);
                        memset(szLongFileNameTemp, 0, MAX_LENGTH);*/
                        sLongFileName = L"";
                        sLongFileNameTemp = L"";
                        ...
                        file.modifiedTime = DateToString(nSFN.modifiedTime);
...
int main(int argc, char** argv)
{
        ...
        case 2: {
            ...
            case 1: {
                ...
                if (f.fileAttributes == "Directory") {
                    ...
                    vector<FileInfo> container = showRootDirectoryOrSubFolder(clusters_sub, _bpb);
                    for (int i = 0; i < container.size(); i++) {
                        cout << "Number " << i << endl;
                        container[i].Print();
                    }
                ...
                if (f.fileAttributes == "Archive") {
                    ...
                    if (pos != wstring::npos) {
                        ...
                        cout << "Need compatible software to open ! \n";
                    }
                }
                break;
    ...
    // memset(sector, 0, 512);
    // 
    // ReadSector(L"\\\\.\\E:", (8192+128*10) * 512, sector);
    // for (auto& i : sector) {
    //     printf("%1c", i);
    // }
    ///* memset(sector, 0, 512);
    // ReadSector(L"\\\\.\\E:", 8193 * 512, sector);
    // for (auto& i : sector) {
    //     printf("%4x", i);
    // }*/
